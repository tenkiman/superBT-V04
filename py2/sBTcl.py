...
class EnsModel(Model,Grib1,Grib2):
    ...
    def __init__(self,
        ...
        # -- 202603015 -- hardwire to a single year
        #
        ndtgs=len(self.dtgs)
        byear=self.bdtg[0:4]
        eyear=self.edtg[0:4]
        
        dyear=int(eyear)-int(byear)
        if(dyear == 0):
            tyear=byear
        elif(dyear == 1):
            if(ndtgs > 365):
                ...
                tyear=eyear
        elif(dyear == 2 and ndtgs > 365):
            tyear=str(int(byear)+1)
        else:
            tyear=byear
            
        newbdtg=tyear
        self.tbaseCtl=self.tbase.replace(self.bdtg,newbdtg)
        tdir=tdir.replace(byear,tyear)
