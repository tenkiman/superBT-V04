    ...
    def chkIfJobIsRunning(self,job,jobopt=None,killjob=1,verb=0,incron=0,
        ...
        def isRunning(job,jobopt,killjob,rcPid=0):
            ...
            for pid in pids:
                ...
                # -- uninteresting procs
                # -- 20240609 -- modern linux runs the mf.runcmd() with 'sh -c'
                if(mf.find(prc,'[') and mf.find(prc,']') or mf.find(prc,'/dev/tty') or mf.find(prc,'-tcsh') or mf.find(prc,'-bash') or
                   ...
                   mf.find(prc,'sbin') or mf.find(prc,'automount') or mf.find(prc,'crond') or mf.find(prc,'sshd') or
                   mf.find(prc,'sh -c')
                   ): continue
...
20240609 -- modern linux ps -ef includes 'sh -c $job' with 'python $job'
...
"""
            ...
            for pid in pids:
                ...
                if(mf.find(prc,job)):
                    if(mf.find(prc,'cron') or mf.find(prc,'sh -c')): continue
                    #print 'ppppppp----wwww',pid,cpid,prc,job
...
class DataSet(MFbase):
    ...
    from mfbase3 import ptmpBaseDir
...
"""%(argdoc,optdoc,purposedoc,exampledoc)
    ...
    def setDefaults(self):
        ...
        for k in list(self.defaults.keys()):
            ...
            elif(type(self.defaults[k]) == int):
                estr="%s=%d"%(k,self.defaults[k])
            elif(type(self.defaults[k]) == float):
