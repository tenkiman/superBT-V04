...
def dtginc(idtg,off):
    ...
    #
    #print 'yyy---',yr,IsLeapYear(yr),idtg,off
...
def dtgmndiff(dtgmn1,dtgmn2):
    ...
    # -- case for very old data... e.g., era5 1940-1980
    #
...
#
    ...
    if(yyyy1%4 == 0 and yyyy2%4 == 0):
        offyear=1980
    if(yyyy2%4 == 0 and (yyyy1%4 != 0 and yyyy2 != 1960)):
        offyear=1979
...
def runcmd(command,logpath='straightrun',lsopt='',prefix='',postfix='',
    ...
    def promptCmd(command):
        text=input('do you want to run: %s Y|N? '%(command))
        if(text=='Y' or text == 'y'):
            os.system(command)
            return
        else:
...
def LsPids():
    ...
    for i in range(1,len(processes)):
        ...
        piddate=tt[4]
        pname=pname.lstrip()
        #print 'pppp',piddate,tt[1],tt[2]
        #print 'ppp-name',pname
