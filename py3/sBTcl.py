...
class MFbase():
    ...
    def ls(self,findstr=None,lsopt=None,maxchar=116,varsonly=0,quiet=0):
        ...
        for d in dd:
            ...
            if(type(d[1]) == list and len(str(d[1])) > maxchar):
                val=str(d[1])[0:maxchar]+' ... '
            elif(type(d[1]) == dict and len(str(d[1])) > maxchar):
                val=str(d[1])[0:maxchar]+' ... '
            else:
                try:
                    val=str(d[1])
    ...
    def dTime(self,tag='notag'):
        ...
        if(not(hasattr(self,'curdtimes'))):
            ...
            if( type(self.curdtimes) != dict ): self.curdtimes={}
...
#                 outconv='grads_grib',
        ...
        if(taus == None):
            self.taus=list(range(btau,etau+1,dtau))
        else:
            self.taus=taus
...
"""%(argdoc,optdoc,purposedoc,exampledoc)
    ...
    def setFlagOpts(self):
        ...
        self.estrFlag=''
        for k in list(self.foptions.keys()):
            if(type(self.foptions[k]) == str):
                estr="%s='%s'"%(k,self.foptions[k])
            elif(type(self.foptions[k]) == int):
                estr="%s=%d"%(k,self.foptions[k])
            elif(type(self.foptions[k]) == float):
...
class Mdeck3(MFutils):
    ...
    def __init__(self,
        ...
        if(not(doSumOnly)):
            ...
            md3dtgs=list(dtgMd3.keys())
            md3dtgs.sort()
            self.md3dtgs=md3dtgs
            self.dtgMd3=dtgMd3
            md3stmids=list(stmMd3.keys())
            md3stmids.sort()
    ...
    def getMd3Stmids(self,stmopt,yearopt=None,dobt=0,dofilt9x=0,verb=0):
        ...
        # -- like tcnames hash
        #
        tcnames=list(self.tcNamesHash.keys())
        ...
        def getstmids(sss,year,dos3id=0,dofilt9x=dofilt9x):
            ...
            if(len(tt) > 1):
                ...
                if(dos3id and len(sss) == 3):
                    ...
                    if(type(bnum1) == str or 
                       (type(bnum1) == int and (bnum1 >= 0 and bnum2 <=9)) or
                       (type(bnum2) == str and bnum2 == 'X')
                       ):
                        doChk9x=1
                ...
                for tcname in tcnames:
                    ...
                    if(chk0):
                        ...
                        if(doChkNum): 
                            ...
                            chk90=(type(bnum1) == int and bnum1 == 9 and type(tcbnum1) == str)
    
                            chk91=(type(bnum1) == str and type(tcbnum1) == str and
                                   (bnum1 == tcbnum1))
                            chk92=(type(bnum2) == int and type(tcbnum2) == int and
                                   (bnum2 == tcbnum2))
                            
                            chk93=(type(bnum1) == int and bnum1 == 9 and 
                                   type(bnum2) == str and bnum2 == 'X' and
                                   not(IsNN(tcstmid)))
...
"""
        ...
        for tau in itaus:
            ...
            if(type(tau) == int and int(tau)%idtmax != 0): continue
            ...
            if(itrkLen == 5):
                fextra1=itrk[tau][-2]
                if(fextra1 != None and not( ( (type(fextra1) == str) and len(fextra1) == 2 ) ) ): doextra1=1
            
            if(itrkLen == 6):
                fextra2=itrk[tau][-1]
                if(fextra2 != None and not( ( (type(fextra2) == str) and len(fextra2) == 2 ) ) ): doextra2=1
        ...
        for tau in itaus:
            ...
            if(type(tau) == int and int(tau)%idtmax != 0): 
        ...
        try:
            otaus=list(range(btau,etaux+1,dtx))
        except:
    ...
    def BiasCorrFcTrackInterpFill(self,jtrk,itrk,deftaus,phr,dtx,
        ...
        xtaus=list(range(taus[-1],taus[-1]+phr,dtx))
...
class Adeck(ADutils):
    ...
    def isValidB2id(self,b2id):
        ...
        rc=0
        if(b2id.upper() in list(Basin2toBasin1.keys())): rc=1
        return(rc)
...
#
    ...
    def relabelAidcards(self,warn=1):
        ...
        for (aid,stm2id) in list(self.aiddtgs.keys()):
    ...
    def cmpAiddtgs(self,Bdtgs,Adtgs):
        astms=list(Adtgs.keys())
        bstms=list(Bdtgs.keys())
...
class f77GridOutput(MFbase):
    ...
    def initVars(self,undef=1e20):
        ...
        if(self.taus == None):
            ...
            self.dtau=6
            self.tunits='hr'
            self.taus=list(range(self.btau,self.etau+1,self.dtau))
...
class AidTrk(MFbase):
    ...
    def __init__(self,dtgs=None,trks=None):
        ...
        if(trks == None):
            ...
            otrks={}
            odtgs=[]
            for dtg in list(trks.keys()):
                odtgs.append(dtg)

                dd=trks[dtg]
                odd={}
                for d in list(dd.keys()):
                    odd[d]=list(dd[d])
...
#     01W.2019', '', '11.2', '125.6', '20', '', '183', '9', '195', '8', '', '', '', '', '', '', '', '', '', '', 'NT', 'NW', 'c', 'c', '', '', '0.27', '', '', '', '', '---']   
    ...
    def lsDSsDtgs(self,dtgs=None,dobt=0,dupchk=0,verb=0,selectNN=1,countsOnly=0,filtTCs=0,doprint=1):
        ...
        if(dtgs == None):
            ...
            if(not(type(dtgs) == list)): dtgs=[dtgs]
            else: dtgs=dtgs
...
class superBT(Mdeck3):
    ...
    def __init__(self,
        ...
        stmsNon=list(sbtNonDev.keys())
        stmsNon.sort()
        
        stmsDev=list(sbtDev.keys())
        stmsDev.sort
        
        stmsNN=list(sbtNN.keys())
        stmsNN.sort()
    ...
    def setsbtTS(self,stmid,sbtType,btime,doDict=1,warn=0,verb=1):
        ...
        if(verb == 2):
            stimes=list(sbtTS.keys())
            stimes.sort()
            for stime in stimes:
                if(doDict):
                    print('ooo-ddd',stime,list(sbtTS[stime].values()))
                else:
...
"""
    ...
    def makeSbtTSTaxis(self):
        ...
        xtimes=list(range(self.btime,self.etime+1,self.dtime))
        xvals={}
...
"""%(self.gadatFile,self.stmopt,undef,
    ...
    def makeGaNonVDevTS(self,sbtTS,tsType,nvars,verb=0):
        ...
        for nvar in nvars:
            ...
            for nk in nkk:
                ...
                nvvals=sbtTS[nk]
                print('nk:',len(nvvals),list(nvvals.keys()))
    ...
    def setsbtVar(self,stmid,sbtType,doDict=1,doDtgKey=0,warn=0,verb=0):
        ...
        if(verb == 2):
            sns=list(sbtVar.keys())
            sns.sort()
            for sn in sns:
                
                if(doDict):
                    print('ooo-ddd',sdtg,list(sbtTS[sn].values()))
                else:
...
"""%(self.gadatFileAll,self.stmopt,undefVar,
    ...
    def makeGaStnVar(self,sbtvarAll,ovars,sMdesc,doGrads=1,verb=0):
        ...
        for n in range(0,nstms):
            nstm=n+1
            stmid=stmids[n]
            sdtgs=list(sbtvarAll[stmid].keys())
            sdtgs.sort()
            dtgs=dtgs+sdtgs
...
class TcBtTrkPlot(DataSet):
    ...
    def PlotTrk(self,
        ...
        if(doft):
            ...
            itaus=list(range(btau,etau+1,dtau))
...
class Era5(Model2):
    ...
    def getDataTaus(self,dtg):
        taus=list(range(0,120+1,6))+list(range(132,240+1,12))
        return(taus)
